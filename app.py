...
if not CONFIG_VALID:
    ...
    if not LLM_API_KEY:
        missing.append("- `LLM_API_KEY` - OpenAI API key")
        ...
        if message["role"] == "user":
            ...
            with st.container():
                ...
                if not result.get("is_valid", False):
                    # Show summarized answer if available, otherwise show error
                    if result.get("summarized_answer"):
                        st.markdown("### 💬 Answer")
                        st.markdown(f"""
                        <div style="background: linear-gradient(135deg, #1a2234 0%, #0f1419 100%); 
                                    padding: 1.5rem; 
                                    border-radius: 12px; 
                                    border-left: 4px solid #ef4444;
                                    margin-bottom: 1.5rem;">
                            <p style="color: #e5e7eb; font-size: 1.05rem; line-height: 1.6; margin: 0;">
                                {result["summarized_answer"]}
                            </p>
                        </div>
                        """, unsafe_allow_html=True)
                    else:
                ...
                # Query Generated
                    ...
                    # Show summarized answer prominently first
                    if result.get("summarized_answer"):
                        st.markdown("### 💬 Answer")
                        st.markdown(f"""
                        <div style="background: linear-gradient(135deg, #1a2234 0%, #0f1419 100%); 
                    ...
                    if result.get("query_success"):
                        with st.expander("📊 View Raw Query Results", expanded=False):
                            ...
                            if result_type == "empty":
                                ...
                                st.download_button(
                                    ...
                                    mime="text/csv",
                                    key=f"download_btn_{i}_{message.get('timestamp', datetime.now()).timestamp()}"
                                )
                        ...
                        # Query Error - show summarized answer if available
                        if result.get("summarized_answer"):
                            st.markdown("### 💬 Answer")
                            st.markdown(f"""
                            <div style="background: linear-gradient(135deg, #1a2234 0%, #0f1419 100%); 
                    ...
                    # If no summarized answer but we have final_answer, show it
                    if not result.get("summarized_answer") and result.get("final_answer"):
                        st.markdown("### 💬 Answer")
                        st.markdown(f"""
                        <div style="background: linear-gradient(135deg, #1a2234 0%, #0f1419 100%); 
                                    padding: 1.5rem; 
                                    ...
                                    border-left: 4px solid #10b981;
                                    margin-bottom: 1.5rem;">
                            <p style="color: #e5e7eb; font-size: 1.05rem; line-height: 1.6; margin: 0; white-space: pre-wrap;">
                                {result["final_answer"]}
                            </p>
                        </div>
                        """, unsafe_allow_html=True)
