...
class MongoDBConnection:
    ...
    def execute_query(self, query_string: str) -> Tuple[bool, Any]:
        ...
        try:
            ...
            if not collection_name:
                return False, f"Could not parse collection name from query. Query received: {query_string[:200]}"
    ...
    def _parse_query(self, query_string: str) -> Tuple[Optional[str], Optional[str], str]:
        ...
        """
        # Clean up the query string - remove newlines, extra whitespace, and markdown
        query_string = query_string.strip()
        # Remove backticks if present
        query_string = query_string.strip('`')
        # Remove any markdown code block markers
        query_string = re.sub(r'^```[\w]*\n?', '', query_string)
        query_string = re.sub(r'\n?```$', '', query_string)
        # Normalize whitespace
        query_string = ' '.join(query_string.split())
        
        # Pattern to match: db.collection.method(params)
        # This pattern handles multi-line queries and nested brackets
        pattern = r'db\.([a-zA-Z_][a-zA-Z0-9_]*)\.(\w+)\s*\('
        ...
        if match:
            ...
            # Find matching closing parenthesis, handling nested brackets
            bracket_count = 0
            in_string = False
            string_char = None
            end_pos = start_pos
            
            for i in range(start_pos, len(query_string)):
                char = query_string[i]
                
                # Handle string literals
                if char in ('"', "'") and (i == 0 or query_string[i-1] != '\\'):
                    if not in_string:
                        in_string = True
                        string_char = char
            ...
            # Extract parameters (everything between the parentheses)
            params = query_string[start_pos:end_pos].strip()
            
            return collection_name, method, params
        
        # Try a simpler pattern as fallback (without requiring opening paren)
        simple_pattern = r'db\.([a-zA-Z_][a-zA-Z0-9_]*)\.(\w+)'
        simple_match = re.search(simple_pattern, query_string)
        
        if simple_match:
            collection_name = simple_match.group(1)
            method = simple_match.group(2)
            # Try to extract params if they exist
            remaining = query_string[simple_match.end():].strip()
            if remaining.startswith('('):
                # Extract everything after the opening paren until matching closing paren
                start_pos = 1
                bracket_count = 0
                in_string = False
                string_char = None
                ...
                params = remaining[start_pos:end_pos].strip()
                return collection_name, method, params
            else:
                # No params, return empty
                return collection_name, method, ""
